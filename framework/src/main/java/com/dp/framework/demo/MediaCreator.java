...
import com.dp.framework.prototype.Picture;
import com.dp.framework.prototype.Song;
import com.dp.framework.prototype.Video;
...
public class MediaCreator {
	...
	public MediaCreator(ApplicationFramework applicationFramework) {
	...
	private void addPictures(BasicMediaServer mediaServer) {
		Picture picture = (Picture) applicationFramework.getComponent(Media.class, Picture.TYPE);
		...
		picture = (Picture) applicationFramework.getComponent(Media.class, Picture.TYPE);
	...
	private void addVideos(BasicMediaServer mediaServer) {
		Video video = (Video) applicationFramework.getComponent(Media.class, Video.TYPE);
		...
		video = (Video) applicationFramework.getComponent(Media.class, Video.TYPE);
	...
	private void addAlbums(BasicMediaServer mediaServer) {
		...
		Album album = (Album) applicationFramework.getComponent(Media.class, Album.TYPE);
		...
		Song song = (Song) applicationFramework.getComponent(Media.class, Song.TYPE);
		song.setAlbumId(album.getId());
		song.setName("Speak To Me");
		song.setTrack(1);
		mediaServer.addToCatalog(song);

		song = (Song) applicationFramework.getComponent(Media.class, Song.TYPE);
		song.setAlbumId(album.getId());
		...
		// Aja
		album = (Album) applicationFramework.getComponent(Media.class, Album.TYPE);
